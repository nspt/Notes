                             ...
                             const std::filesystem::path& geometryShaderPath)
{
    ...
    try {
        vertex_shader = compileShader(GL_VERTEX_SHADER, vertexSource, vertexShaderPath);
        geometry_shader = has_geom
            ? compileShader(GL_GEOMETRY_SHADER, geometrySource, geometryShaderPath)
            : 0;
        fragment_shader = compileShader(GL_FRAGMENT_SHADER, fragmentSource, fragmentShaderPath);

        data_->id_ = linkProgram(
...
std::string ShaderProgram::readTextFile(const std::filesystem::path& path)
{
    ...
    ss << file.rdbuf();
    return resolveIncludes(ss.str(), path.parent_path());
}

std::string ShaderProgram::resolveIncludes(const std::string &source,
                                           const std::filesystem::path &base_dir,
                                           int depth)
{
    if (depth > 32) {
        throw std::runtime_error("shader #include nesting too deep");
    }
    ...
    while (std::getline(in, line)) {
        ...
        if (trimmed.rfind("#include", 0) == 0) {
            ...
            const auto q2 = trimmed.find('"', q1 == std::string::npos ? q1 : q1 + 1);
            if (q1 == std::string::npos || q2 == std::string::npos || q2 <= q1 + 1) {
                throw std::runtime_error("invalid #include: " + line);
            }
            const auto rel = trimmed.substr(q1 + 1, q2 - q1 - 1);
            const auto inc_path = base_dir / rel;
            std::ifstream inc_file(inc_path, std::ios::in);
            if (!inc_file) {
                throw std::runtime_error("failed to open included shader: " + inc_path.string());
            }
            std::ostringstream inc_ss;
            inc_ss << inc_file.rdbuf();
            out << resolveIncludes(inc_ss.str(), inc_path.parent_path(), depth + 1);
            out << '\n';
            continue;
        }
        out << line << '\n';
                                    ...
                                    const std::filesystem::path& path)
{
    ...
    if (success != GL_TRUE) {
        ...
        std::string msg = "failed to compile " + typeName;
        if (!path.empty()) {
            msg += " [" + path.string() + "]";
        }
        msg += ":\n" + log;
                                  ...
                                  const std::filesystem::path& geometryPath)
{
    ...
    if (success != GL_TRUE) {
        ...
        auto appendPath = [&](const char* label, const std::filesystem::path& p) {
            if (!p.empty()) {
                msg += "\n  ";
                msg += label;
                msg += ": ";
        ...
        appendPath("vertex", vertexPath);
        appendPath("fragment", fragmentPath);
        appendPath("geometry", geometryPath);
        msg += ":\n" + log;
        throw std::runtime_error(msg);
    }
...
{
    auto &map{ data_->uniform_loc_cache_ };
    std::string n{ name };
    auto iter = map.find(n);
    if (iter != map.end()) {
        return iter->second;
    }
    auto loc = glGetUniformLocation(data_->id_, n.c_str());
    map.insert({n, loc});
    return loc;
}
...
{
    auto &map{ data_->uniform_block_loc_cache_ };
    std::string n{ name };
    auto iter = map.find(n);
    if (iter != map.end()) {
        return iter->second;
    }
    auto loc = glGetUniformBlockIndex(data_->id_, n.c_str());
    map.insert({n, loc});
    return loc;
}
