...
mat3 getTBNMatrix()
{
    vec3 T = normalize(fs_in.v_world_tangent);
    vec3 N = normalize(fs_in.v_world_normal);
...
vec3 getFragNormalInWorld()
{
    // 无 normal map 时 C++ 会绑定平坦法线 (0.5,0.5,1) → TBN*(0,0,1)=N
    mat3 tbn_matrix = getTBNMatrix();
    vec3 tangent_normal = texture(material.normal_map, fs_in.v_tex_coord).xyz * 2.0 - 1.0;
