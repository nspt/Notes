...
int main(int argc, char* argv[])
{
    ...
    try {
        ...
        auto general_shader = std::make_shared<ShaderProgram>(
            resourceDir + "shaders/18_instancing/16env_cube.vert",
            resourceDir + "shaders/18_instancing/16env_cube.frag"
        );
        ...
        explode_model.program_ = std::make_shared<ShaderProgram>(
            resourceDir + "shaders/18_instancing/17explode.vert",
            resourceDir + "shaders/18_instancing/17explode.frag",
            resourceDir + "shaders/18_instancing/17explode.geom"
        );
        ...
        quad_obj.material_->program_ = std::make_shared<ShaderProgram>(
            resourceDir + "shaders/18_instancing/15kernel.vert",
            resourceDir + "shaders/18_instancing/15kernel.frag"
        );
        ...
        skybox.material_->program_ = std::make_shared<ShaderProgram>(
            resourceDir + "shaders/18_instancing/16cubemap.vert",
            resourceDir + "shaders/18_instancing/16cubemap.frag"
        );
        ...
        for (unsigned frame = 0; !glfwWindowShouldClose(window); ++frame) {
            auto draw_scene = [&]() {
                ...
                // draw planet
                render(planet, view, win_data->projection);
                for (auto &t : rock_transforms) {
                    rock.transform_ = t;
                    render(rock, view, win_data->projection);
                }
