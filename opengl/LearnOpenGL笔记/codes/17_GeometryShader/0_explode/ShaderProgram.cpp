                             ...
                             const std::filesystem::path& geometryShaderPath)
{
    ...
    const GLuint geometryShader  = has_geom ? compileShader(GL_GEOMETRY_SHADER, geometrySource) : 0;
    const GLuint fragmentShader = compileShader(GL_FRAGMENT_SHADER, fragmentSource);

    try {
        id_ = linkProgram(vertexShader, geometryShader, fragmentShader);
    } catch (...) {
...
GLuint ShaderProgram::uniformBlockLocation(std::string_view name) const
{
    ...
    return glGetUniformBlockIndex(id_, nameStr.c_str());
}
