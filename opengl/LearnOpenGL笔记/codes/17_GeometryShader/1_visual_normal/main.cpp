...
int main(int argc, char* argv[])
{
    ...
    try {
        ...
        auto general_shader = std::make_shared<ShaderProgram>(
            resourceDir + "shaders/17_geometry_shader/16env_cube.vert",
            resourceDir + "shaders/17_geometry_shader/16env_cube.frag"
        );
        ...
        explode_model.program_ = std::make_shared<ShaderProgram>(
            resourceDir + "shaders/17_geometry_shader/17explode.vert",
            resourceDir + "shaders/17_geometry_shader/17explode.frag",
            resourceDir + "shaders/17_geometry_shader/17explode.geom"
        );
        ...
        quad_obj.material_->program_ = std::make_shared<ShaderProgram>(
            resourceDir + "shaders/17_geometry_shader/15kernel.vert",
            resourceDir + "shaders/17_geometry_shader/15kernel.frag"
        );
        ...
        skybox.material_->program_ = std::make_shared<ShaderProgram>(
            resourceDir + "shaders/17_geometry_shader/16cubemap.vert",
            resourceDir + "shaders/17_geometry_shader/16cubemap.frag"
        );

        auto visual_normal_shader = std::make_shared<ShaderProgram>(
            resourceDir + "shaders/17_geometry_shader/17visual_normal.vert",
            resourceDir + "shaders/17_geometry_shader/17visual_normal.frag",
            resourceDir + "shaders/17_geometry_shader/17visual_normal.geom"
        );
        auto pure_color_material = std::make_shared<Material>();
        pure_color_material->pure_color_ = true;
        pure_color_material->color_ = glm::vec3{ 1, 0, 0 };
        ...
        for (unsigned frame = 0; !glfwWindowShouldClose(window); ++frame) {
            auto draw_scene = [&]() {
                ...
                for (auto &obj : model.objects_) {
                    auto m = obj->material_;
                    obj->material_ = pure_color_material;
                    render(*obj, model.transform_, view, win_data->projection, visual_normal_shader);
                    obj->material_ = m;
                }

                glDisable(GL_CULL_FACE);
                explode_model.program_->setFloat("explode_magnitude", (sin(glfwGetTime()) + 1.0) / 2.0 * 2.0);
                render(explode_model, view, win_data->projection);
                glEnable(GL_CULL_FACE);
