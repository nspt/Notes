...
int main(int argc, char* argv[])
{
    ...
    try {
        ...
        // MSAA 纹理不能直接 sampler2D 采样，需 resolve 到普通 Texture2D
        Texture2D mirror_texture{ mirror_w, mirror_h, GL_RGB };
        FrameBuffer mirror_resolve_fbo;
        mirror_resolve_fbo.attachTexture(GL_COLOR_ATTACHMENT0, mirror_texture);
        if (!mirror_resolve_fbo.isCompleted()) {
            std::cerr << "Mirror resolve FBO is not completed" << std::endl;
            abort();
        }

        RenderObject mirror;
        mirror.material_.program_ = ShaderProgram {
            resourceDir + "shaders/19_anti_aliasing/kernel_0.vert",
            resourceDir + "shaders/19_anti_aliasing/kernel_0.frag"
        };
        ...
        for (unsigned frame = 0; !glfwWindowShouldClose(window.get()); ++frame) {
            if (mirror_ms_texture.width() != win_data->win_width
                ...
                mirror_texture.reallocate(w, h, GL_RGB);
            }
            ...
            // resolve MSAA -> Texture2D
            mirror_fbo.bind(GL_READ_FRAMEBUFFER);
            mirror_resolve_fbo.bind(GL_DRAW_FRAMEBUFFER);
            glBlitFramebuffer(
                0, 0, mirror_ms_texture.width(), mirror_ms_texture.height(),
                0, 0, mirror_texture.width(), mirror_texture.height(),
