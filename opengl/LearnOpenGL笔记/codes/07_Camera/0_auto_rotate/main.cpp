...
try {
    ...
    for (unsigned frame = 0; !glfwWindowShouldClose(window); ++frame) {
        auto elapsed = duration_cast<duration<float>>(steady_clock::now() - last_time);
        ...
        float yaw_speed = 360.0f / 10.0f;
        float delta_yaw = elapsed.count() * yaw_speed;
        camera.yaw(delta_yaw);
        auto camPos = glm::rotate(glm::mat4{ 1.0f }, glm::radians(camera.yaw()), glm::vec3{ 0.0f, 1.0f, 0.0f })
            * glm::vec4{ 0.0f, 0.0f, 10.0f, 1.0f };
        camera.moveTo(glm::vec3{ camPos.x, camPos.y, camPos.z });
        shaderProg.setMat4("viewTrans", camera.viewMatrix());
