...
TextureCubeMap::TextureCubeMap(int width, int height, GLenum internalFormat, GLenum format)
    : Texture{ GL_TEXTURE_CUBE_MAP }
{
    init({}, width, height, internalFormat, format);
}

TextureCubeMap::TextureCubeMap(int width, int height, GLenum format)
    : Texture{ GL_TEXTURE_CUBE_MAP }
{
    init({}, width, height, format, format);
}

void TextureCubeMap::init(const std::vector<void*> faces, int width, int height, GLenum internalFormat, GLenum format) noexcept
{
    data_->width_ = width;
    ...
    for(unsigned int i = 0; i < faces.size(); i++)
    {
        glTexImage2D(
            ...
            0, internalFormat, width, height, 0, format, GL_UNSIGNED_BYTE, faces[i]
        );
    }
