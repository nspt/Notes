...
Texture2D::Texture2D(int width, int height, GLenum internalFormat, GLenum format, void *data)
    : Texture{ GL_TEXTURE_2D }
{
    init(data, width, height, internalFormat, format);
}

Texture2D::Texture2D(int width, int height, GLenum format, void *data)
    : Texture{ GL_TEXTURE_2D }
{
    init(data, width, height, format, format);
}
...
void Texture2D::reallocate(int width, int height, GLenum internalFormat, GLenum format, void *data)
{
    data_->width_ = width;
    ...
    glTexImage2D(GL_TEXTURE_2D, 0,
        internalFormat, data_->width_, data_->height_, 0,
...
{
    reallocate(width, height, format, format, data);
}

void Texture2D::init(void *data, int width, int height, GLenum internalFormat, GLenum format) noexcept
{
    data_->width_ = width;
    data_->height_ = height;
    data_->format_ = format;
