...
int main(int argc, char* argv[])
{
    try {
        ...
        ShaderProgram objShaderProg {
            resourceDir + "shaders/08_lighting/08phong.vert.glsl",
            resourceDir + "shaders/08_lighting/08phong.frag.glsl"
        };
        ...
        for (unsigned frame = 0; !glfwWindowShouldClose(window); ++frame) {
            ...
            lightPos = glm::vec3(lightRotateMat * glm::vec4{ lightPos, 1.0f });
    
            objShaderProg.use();
            brickTexture.bind(0);
            faceTexture.bind(1);
    
            objShaderProg.setVec3("lightPos", lightPos);
            objShaderProg.setVec3("viewPos", win_data->camera.pos());
            objShaderProg.setMat4("viewTrans", win_data->camera.viewMatrix());

            glm::mat4 modelTrans{ 1.0 };
            //modelTrans = glm::rotate(modelTrans, glm::radians(45.0f), glm::vec3{ 0.0f, 1.0f, 0.0f });
            ...
            lightModelTrans = glm::scale(lightModelTrans, glm::vec3{ 0.1f, 0.1f, 0.1f });
