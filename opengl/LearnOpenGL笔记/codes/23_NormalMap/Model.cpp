...
RenderObject Model::processMesh(const aiMesh *mesh, const aiScene *scene, const std::string_view &dir)
{
    ...
    if(mesh->mMaterialIndex < scene->mNumMaterials) {
        ...
        material.normal_textures_ = loadTextureFrom(m, scene, aiTextureType_NORMALS, dir, false);
        if (material.normal_textures_.empty()) {
            // 部分模型用 HEIGHT 槽位存法线贴图
            material.normal_textures_ = loadTextureFrom(m, scene, aiTextureType_HEIGHT, dir, false);
        }
