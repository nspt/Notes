...
void framebufferSizeCallback(GLFWwindow* window, int width, int height)
{
    ...
    data->cam_data.projection = glm::perspective(
        ...
        static_cast<float>(width / 2.0) / static_cast<float>(height),
        0.1f,
        100000.0f
    );
    data->cam_data_UBO.setSubData(0, sizeof(CameraData), &data->cam_data);
...
void scroll_callback(GLFWwindow* window, double xoffset, double yoffset)
{
    ...
    data->cam_data.projection = glm::perspective(
        glm::radians(data->fov),
        static_cast<float>(data->win_width / 2.0) / static_cast<float>(data->win_height),
        0.1f,
        100000.0f
    );
    data->cam_data_UBO.setSubData(0, sizeof(CameraData), &data->cam_data);
...
void initLightData(WinData &win_data)
{
    ...
    data.directional[0].diffuse_ = glm::vec4{ 0.0 };
    data.directional[0].specular_ = glm::vec4{ 0.0 };
    ...
    // 3. spot
    data.counts.z = 1;
    data.spot[0].pos_ = glm::vec4{ 0, 5, 3, 1 };
    {
    ...
    data.spot[0].diffuse_ = glm::vec4{ 0.0 };
...
void initWinData(WinData &win_data)
{
    ...
    win_data.cam_data.projection = glm::perspective(
        ...
        static_cast<float>(win_data.win_width / 2.0) / static_cast<float>(win_data.win_height),
        0.1f,
        100000.0f
    );
...
int main(int argc, char* argv[])
{
    ...
    try {
        ...
        ShaderProgram phong_shader{
            resourceDir + "shaders/20_blinn_phong/phong.vert",
            resourceDir + "shaders/20_blinn_phong/phong.frag"
        };
        ...
        ShaderProgram blinn_phong_shader{
            resourceDir + "shaders/20_blinn_phong/blinn_phong.vert",
            resourceDir + "shaders/20_blinn_phong/blinn_phong.frag"
        };
        ...
        ShaderProgram skybox_shader{
            resourceDir + "shaders/20_blinn_phong/skybox.vert",
            resourceDir + "shaders/20_blinn_phong/skybox.frag"
        };
        skybox_shader.setUniformBlockBinding("CamData", 1);
        skybox_shader.setInt("skybox", 0);

        objects.push_back(createPlatform(resourceDir, phong_shader));
        objects.front().objects_[0].material_.specular_textures_.clear();
        objects.front().objects_[0].material_.specular_textures_.push_back(
            objects.front().objects_[0].material_.diffuse_textures_.front()
        );

        Model flashlight{ resourceDir + "/model/flash_light", "Flashlight.obj", phong_shader, false };
        ...
        for (unsigned frame = 0; !glfwWindowShouldClose(window.get()); ++frame) {
            ...
            glViewport(0, 0, win_data->win_width / 2, win_data->win_height);
            for (auto &m : objects) {
                render(m, &phong_shader);
            }
            // draw skybox
            glDisable(GL_CULL_FACE);
            glDepthFunc(GL_LEQUAL);
            skybox.material_.diffuse_textures_[0].bind(0);
            skybox_shader.setMat4("no_translate_view", glm::mat4(glm::mat3(win_data->cam_data.view)));
            skybox_shader.use();
            skybox.mesh_.draw();
            glDepthFunc(GL_LESS);
            glEnable(GL_CULL_FACE);
            ...
            glViewport(win_data->win_width / 2, 0, win_data->win_width / 2, win_data->win_height);
            for (auto &m : objects) {
                render(m, &blinn_phong_shader);
            }
            // draw skybox
            glDisable(GL_CULL_FACE);
            glDepthFunc(GL_LEQUAL);
