...
Depth Buffer 是类似 Color Buffer 的一个缓冲区，有着同样的宽高，只是存储的是片段的深度值，常见精度有 16 位、24 位和 32 位。
...
对于透视投影，视图空间中的 z 在经过投影矩阵和透视除法后，会非线性地映射到深度缓冲区，因此深度值与 $\frac{1}{z}$ 成正比。深度值的大部分精度集中分布在靠近近平面的区域，这使得我们能够对附近的物体获得较高的深度精度。
...
Z-Fighting 是指两个片段的深度值过于接近，超过深度缓冲区的能够区分的精度，导致深度测试无法稳定判断前后关系，从而出现表面闪烁、交替显示等现象。
