...
要搞清楚伽马校正，就得从 CRT 显示器说起。CRT 显示器的显示亮度（Luminance）和其输入信号/电压（Voltage）并非线性关系，而是幂律关系：
...
其中指数伽马 $\gamma$ 大约为 2.2，由于 CRT 的这个特性，图像的中低亮度区域会变得更暗，导致暗部太暗而看不清楚，下面分别是图像的预期效果和最终显示效果：

![original](images/moncalib_gamma22b.jpg)

![crt_display](images/moncalib_gamma40b.jpg)
...
![gamma_corrected](images/moncalib_gamma10b.jpg)

经过伽马校正后的输入，再交给 CRT 显示器显示时，就能还原回线性的光强：

$$
L \propto V ^ \gamma \rightarrow L \propto (L_0^{1/\gamma}) ^ \gamma \rightarrow L \propto L_0
$$

![gamma_correction_process](images/gamma_correction_process.jpg)
...
显示器默认存在的伽马解码，以及大量历史图像数据，又迫使新的图像（来源于拍照、绘制等）也不得不主动完成伽马编码。实际上，“伽马校正”已经是 sRGB 标准的要求，也就是说 JPG、PNG 图像中的颜色值通常都是已经伽马编码过的值。

> sRGB 标准并不是直接按照 $\frac{1}{2.2}$ 次幂定义的伽马校正，而是一个分段函数，目的是使用更合理的（更适合人眼感知）的编码。显示器实际上也是按 sRGB 标准进行解码。

# 伽马校正与人眼感知的关联

Gamma 编码最初是为了补偿 CRT 的非线性响应，但这种非线性编码同时具有提高感知和编码效率的效果，因此后来被图像系统广泛采用。

在人眼的视觉范围内，人眼对相对光强变化更敏感，因此在暗部，相同的绝对光强变化通常会带来更明显的主观亮度变化。也就是说，**人眼对暗部的相对光强变化更敏感，对亮部的相同绝对光强变化不那么敏感**。
...
借助 `glEnable(GL_FRAMEBUFFER_SRGB);` 让 OpenGL 自动进行转换，仅在当前绑定的 FBO 的 color attachment 是 sRGB 格式时会生效。可以通过以下方式查询：
...
if (colorEncoding == GL_SRGB)
    std::cout << "sRGB\n";
else if (colorEncoding == GL_LINEAR)
    std::cout << "Linear\n";
```

> 某些驱动/窗口系统会违反此规定，即便默认 FBO 的颜色附件是 `GL_LINEAR`，依然支持 `GL_FRAMEBUFFER_SRGB` 的使能效果。

如果需要自定义伽马值，或者希望伽马校正的位置和行为更加明确、可控，也可以不借助 OpenGL 的内置功能，而在 Fragment Shader 中自行完成：
