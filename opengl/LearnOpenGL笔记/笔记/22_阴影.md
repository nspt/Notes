...
上图表示的是一个被倾斜光照射的平面，倾斜黄线表示阴影贴图中的同一个纹素，也表示着纹素的深度。因为阴影贴图分辨率不足，水平的黄线和粗黑线表示的片段，对应了同一个阴影贴图纹素，但是两者的深度不同（因为光线和平面非垂直），一个小于纹素深度、被点亮、标为黄色，一个大于纹素深度、未被点亮、标为黑色。最终呈现的就是类似摩尔纹的 Shadow Acne。当光线以越倾斜的角度照射平面，即平面和光线越接近于平行时，同一个 Shadow Map 纹素覆盖区域内的深度差异越大，越容易产生 Shadow Acne。

为了解决 Shadow Acne，需要引入一个小技巧： *Shadow Bias*，就是将采样得到的深度值，加上一点偏移值后，也就是让阴影贴图中记录的深度值“变大一点”后，再与片段比较。也就是允许当前片段的深度比 Shadow Map 中的深度略大一小段距离，只有超过这个容差才认为处于阴影中：
...
这种做法对于有“厚度”的物体比较有效果，其本质上是另一种形式的 bias，对于具有厚度的封闭物体，使用背面生成 Shadow Map 通常可以让记录的深度相对于正面更“靠后”，相当于将阴影贴图中的深度值加大了（使用背面的深度值），从而提供一定的天然深度容差，减少 Shadow Acne，但因为是背面，所以不影响对比结果：

![culling](images/shadow_mapping_culling.png)

## 阴影贴图采样出界（Over Sampling）

光源的投影矩阵定义了光源的照亮范围，并非所有物体都会落在此范围内，对于不在照亮范围内的片段，该如何识别及处理就成了一个问题，也就是采样出界（*Over Sampling*）。

对于不在光源空间视椎体（Frustum）内的片段，其阴影坐标会超出 $[0, 1]$ 范围，对于纹理坐标（X/Y）的越界，我们可以通过设置纹理的 wrap mode 为 `GL_CLAMP_TO_BORDER` 并提供 border color 来使其采样值为指定值：
...
增加阴影贴图的分辨率、尽可能让光的视锥体贴合场景都可以减轻阴影锯齿，除此之外，还有软化阴影边缘、降低锯齿视觉效果的算法：百分比渐进滤波（*percentage-closer filtering，PCF*）。PCF 通过对阴影贴图周围多个纹素分别进行深度比较，再将得到的 0/1 结果取平均，从而得到平滑的阴影边缘：
...
我们可以借助立方体贴图（Cubemap Texture）来实现点光阴影，这种技术也叫万象阴影贴图（Omnidirectional Shadow Map）。
...
```cpp
for(int i = 0; i < 6; i++) {
    // 每次只把 Cubemap 的一个面附加到 FBO，然后渲染一次。
...
每个面的 view 变换如下，注意 up 向量的设定是为了保证渲染结果与 Cubemap 六个面的坐标约定一致：
...
因为点光阴影是立方体贴图，所以抗锯齿的 PCF 算法不能直接套用 2D 纹理的做法，我们需要扩展为 3D 的做法，对原始光源到片段方向附近的多个采样方向进行采样：
...
但是这种做法性能代价很大，当 `samples` 为 4 时，总采样数达到了 64。实际上这些采样点存在冗余，其中一些采样方向与原始方向向量非常接近，而沿原始方向向量的垂直方向进行偏移后采样更为合理。如果我们能去掉冗余的采样，就可以减少采样次数。然而，确定哪些子方向存在冗余并不容易，所以我们可以采用的一个技巧是使用一个偏移方向数组，其中所有方向互不重叠，例如每个方向都指向完全不同的方向，来减少最终相邻的子方向数量。下面是一个包含 20 个偏移方向的数组，这些坐标是一个立方体的 8 个顶角和 12 条边的中点，这些方向不是严格意义上的均匀球面采样，而是一种简单、经验性的采样模式：
